...
var pP=((2n**64n)-83n),m=2**31,tp,i=0,x=bd.length,ac=0n;
for (i in bd){
	bd[i]=ac;
	tp=BigInt(k[0]*(k[4]%m-i))%pP
	bd[i]=(bd[i]+tp)%pP;
	tp=BigInt(k[1]*(k[5]%m-i+1))%pP
	bd[i]=(bd[i]+tp)%pP;
	tp=BigInt(k[2]*(k[6]%m-i+3))%pP;
	bd[i]=(bd[i]+tp)%pP;
	tp=BigInt(k[3]*(k[7]%m-i+2))%pP;
bd[i]=(bd[i]+tp)%pP;
ac=bd[i]}
return bd}
